...
void playBlackjack(int* playerChips, int numberOfPlayers) {
    ...
    int init_dealt_card_first = dealCard(&dealerTotal, &deckPosition, &dealerAceCount, 1);
    ...
    *init_done_ptr = 1; // set init done to 1 to indicate memory is full
    ...
    int init_dealt_card = dealCard(&dealerTotal, &deckPosition, &dealerAceCount, 1);
    Card hidden_card = deck[deckPosition - 1];
    dealerTotal += init_dealt_card;
    ...
    if (!dealerBJ){
        ...
        for (int i = 0; i < numberOfPlayers; i++) {
            ...
            *(dealer_top_ptr) = dealer_initial;
            for (int f = *(mem_starting_ptr); f < DECK_SIZE; f++) { // filling memory
    ...
    for (int i = 0; i < numberOfPlayers; i++) {
        if(blackjack[i] == 1 && !dealerBJ){
            ...
            //Game_text_clear(13,19,50,52);
            //Game_text_clear(13+20,19+20,50,52);
            //Game_text_clear(13+40,19+40,50,52);
            //Game_text_clear(13+60,19+60,50,52);
