...
/// Audio
/// compile with
/// gcc audio_drum.c -o testA -lm -O3
...
int main(void)
{
	...
	while(1){	
      ...
      rhoeff = float2fix28(MIN(0.49, RHO_0 + pow(fix2float28(drum_n[drum_middle][drum_middle] >> 4), 2))); //nonlinearly scaling rho to handle week 1
          ...
          new_drum_temp = multfix28(rhoeff, (drum_n[i-1][j] + drum_n[i+1][j] + drum_n[i][j-1] + drum_n[i][j+1] - times4pt0(drum_n[i][j])));              
